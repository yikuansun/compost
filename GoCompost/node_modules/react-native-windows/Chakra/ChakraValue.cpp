...
ChakraObject ChakraValue::asObject() {
  ...
  if (result != JsNoError) {
    ...
    if (hasException) {
      ...
      throwJSExecutionException("JsConvertValueToObject() failed for reason other than script exception.");
    }
  }
...
ChakraValue ChakraObject::callAsFunction(std::initializer_list<JsValueRef> args) const {
  // OFFICEDEV
...
ChakraValue ChakraObject::callAsFunction(const ChakraObject &thisObj, int nArgs, const JsValueRef args[]) const {
  return callAsFunction(static_cast<JsValueRef>(thisObj), nArgs, args);
}

ChakraValue ChakraObject::callAsFunction(JsValueRef thisObj, int nArgs, const JsValueRef args[]) const {
  JsValueRef exn = nullptr;
  ...
  JsValueRef value;
  auto result =
      JsCallFunction(m_obj, const_cast<JsValueRef *>(argsWithThis), static_cast<unsigned short>(nArgs + 1), &value);
  delete[] argsWithThis;
  if (result != JsNoError) {
    bool hasException = false;
    if (JsHasException(&hasException) != JsNoError) {
    ...
    if (hasException) {
      ...
      throwJSExecutionException("JsCallFunction() failed for reason other than script exception.");
    }
  }
...
ChakraObject ChakraObject::callAsConstructor(std::initializer_list<JsValueRef> args) const {
  JsValueRef exn = nullptr;

  JsValueRef value;
  auto result = JsConstructObject(
      m_obj, const_cast<JsValueRef *>(args.begin()), static_cast<unsigned short>(args.size()), &value);
  ...
  if (result != JsNoError) {
    ...
    if (hasException) {
      ...
      throwJSExecutionException("JsConstructObject() failed for reason other than script exception.");
    }
  }
...
void ChakraObject::setProperty(const char *propName, const ChakraValue &value) const {
